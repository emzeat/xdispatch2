...
)
{
    ...
    auto work = xdispatch::make_operation( [&passes]
    {
        ++passes;
    } );
    ...
    MU_MESSAGE( "Dispatched %i operations, %i nsec per operation", actual, watch_dispatch.elapsed() * 1000 / actual );
    MU_MESSAGE( "Executed %i operations, %i nsec per operation", actual, watch_execution.elapsed() * 1000 / actual );
}

void cxx_benchmark_serial_queue(
    void* data
)
{
...
void cxx_benchmark_global_queue(
    void* data
)
{
...
)
{
    ...
    group.notify( [&watch, &passes]
    {
        watch.stop();
        const int actual = passes;
        MU_MESSAGE( "Dispatch %i operations, %i nsec per operation", actual, watch.elapsed() * 1000 / actual );
        MU_PASS( "Test completed" );
    }, queue );
