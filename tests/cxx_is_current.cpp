...
void
cxx_is_current(void* data)
{
    CXX_BEGIN_BACKEND_TEST(platform_test_main_queue);
    auto group = cxx_create_group();

    group.async([] { MU_ASSERT_TRUE(cxx_main_queue().is_current_queue()); },
                cxx_main_queue());

    group.async(
      [] {
          MU_ASSERT_TRUE(cxx_global_queue(xdispatch::queue_priority::BACKGROUND)
                           .is_current_queue());
      },
    ...
    group.async(
      [] {
          MU_ASSERT_TRUE(cxx_global_queue(xdispatch::queue_priority::UTILITY)
                           .is_current_queue());
      },
    ...
    group.async(
      [] {
          MU_ASSERT_TRUE(cxx_global_queue(xdispatch::queue_priority::DEFAULT)
                           .is_current_queue());
      },
    ...
    group.notify([] { MU_PASS("Seems to work"); }, cxx_main_queue());
