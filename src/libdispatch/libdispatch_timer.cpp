...
public:
    ...
    ) final
    {
        ...
        case timer_precision::COARSE:
            m_latency = NSEC_PER_SEC;
            break;
        case timer_precision::DEFAULT:
            m_latency = 5 * NSEC_PER_MSEC;
            break;
    ...
    ) final
    {
        ...
        {
            m_delay = dispatch_time( DISPATCH_TIME_NOW, delay.count() * NSEC_PER_MSEC );
        }
...
{
    object_scope_T< dispatch_source_t > native( dispatch_source_create( DISPATCH_SOURCE_TYPE_TIMER, 0, 0, impl_2_native( queue ) ) );
